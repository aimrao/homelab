...
def download_playlist(playlist_url):
    try:
        # Step 1: Extract playlist info using a temporary configuration.
        temp_opts = {
            'quiet': True,
            'skip_download': True,
        }
        ...
        # Step 2: Create the folder for the playlist if it doesn't exist.
        folder = os.path.join('/data', playlist_title)
        if not os.path.exists(folder):
            os.makedirs(folder)
        
        # Step 3: Update the options with the resolved playlist title.
        ...
        ydl_opts = {
            ...
            'external_downloader_args': ['-x', '16', '-k', '100M'],
        }
        
        # Step 4: Download the playlist using the updated options.
